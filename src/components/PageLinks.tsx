    ...
    <nav aria-label="Explore" className="divide-y divide-white/10 border-y border-white/10">
      {items.map((item, i) => (
        <Reveal key={item.href} delay={i * 80}>
          ...
          >
            <span className="text-3xl font-extralight uppercase tracking-[0.12em] text-white/75 transition-all duration-500 group-hover:translate-x-3 group-hover:text-magenta sm:text-5xl md:text-6xl">
              {item.label}
            </span>
            <span
              aria-hidden
              className="translate-x-0 text-2xl text-white/30 transition-all duration-500 group-hover:translate-x-2 group-hover:text-magenta sm:text-4xl"
            >
              →
            </span>
          </Link>
